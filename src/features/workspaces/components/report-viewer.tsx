...
export default function ReportViewer({ initialFileKey, workspaceSlug, username }: ReportViewerProps) {
      ...
      s.events.forEach(e => {
        if (e.findings) {
          ...
          e.findings.forEach(f => {
             if (f.severity === 'high' || f.severity === 'critical') findingsHigh++;
          });
          // Dummy risk scoring
        ...
        } else if (e.type === 'response_item' && e.payload?.type === 'token_count') {
    ...
    return (
      <div className="bg-[#09090b] text-white flex flex-col items-center justify-center p-4 min-h-[60vh]">
        <Card className="w-full max-w-md bg-zinc-900 border-zinc-800 text-center p-8">
          ...
          <h1 className="text-2xl font-bold mb-2 text-white">Security Report Analyzer</h1>
          <p className="text-zinc-400 mb-8">Upload your <code>report.json</code> file to visualize agent interactions, risk findings, and token usage.</p>
    ...
    <div className="bg-[#09090b] text-zinc-100 p-6 flex flex-col gap-6 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm font-mono mt-1">
            Generated: {stats?.lastGenerated ? format(new Date(stats.lastGenerated), 'PPpp') : '-'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatsCard 
          ...
          icon={<Activity className={cn("text-zinc-500", (stats?.maxRisk ?? 0) > 0 && "text-red-500")} />} 
          className={(stats?.maxRisk ?? 0) > 0 ? "border-red-500/30 bg-red-500/5" : ""}
        />
        <StatsCard 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="bg-zinc-900 border-zinc-800">
          <CardHeader>
            <CardTitle className="text-zinc-100">Risk Gradient Over Time</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          <CardHeader>
             <CardTitle className="text-zinc-100">Token Usage per Session</CardTitle>
          </CardHeader>
          <CardContent>
             <div className="h-[200px] w-full">
               <ResponsiveContainer width="100%" height="100%">
          ...
          <CardHeader className="pb-3 border-b border-zinc-800">
             <div className="flex items-center justify-between">
                ...
                <div className="relative w-32">
                   ...
                   <Input placeholder="Filter..." className="h-8 pl-8 text-xs bg-zinc-950 border-zinc-800 text-zinc-200 focus:ring-emerald-500/20" />
                </div>
             </div>
          </CardHeader>
                ...
                {stats?.sessionStats.map((s) => (
                   ...
                   >
                     <div className="flex justify-between items-center mb-1">
                        ...
                        {s.risk > 0 && (
                          <Badge variant="outline" className="text-[10px] h-5 border-red-900/50 text-red-500 bg-red-900/10">
                            RISK: {s.risk}
                          </Badge>
                        )}
                     </div>
            ...
            <>
              <div className="p-4 border-b border-zinc-800 bg-zinc-950/30 flex justify-between items-center">
                 <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-sm text-zinc-300">Session: {selectedSession.sessionId}</span>
                 </div>
                 <div className="flex gap-2">
              ...
              <Tabs defaultValue="chat" className="flex-1 flex flex-col overflow-hidden">
                <TabsList className="w-full justify-start rounded-none border-b border-zinc-800 bg-transparent px-4 h-11">
                  <TabsTrigger value="chat" className="data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-emerald-500 data-[state=active]:text-emerald-400 text-zinc-400 rounded-none h-11">
                    <MessageSquare className="w-4 h-4 mr-2" />
                    Chat Flow
                  </TabsTrigger>
                  <TabsTrigger value="metadata" className="data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-blue-500 data-[state=active]:text-blue-400 text-zinc-400 rounded-none h-11">
                    <Activity className="w-4 h-4 mr-2" />
                    Context & Meta
                  </TabsTrigger>
                  <TabsTrigger value="findings" className="data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-red-500 data-[state=active]:text-red-400 text-zinc-400 rounded-none h-11">
                     <AlertTriangle className="w-4 h-4 mr-2" />
                     Findings <span className="ml-2 bg-red-500/20 text-red-400 px-1.5 rounded-full text-xs">{selectedSession.events.reduce((acc, e) => acc + (e.findings?.length || 0), 0)}</span>
                  </TabsTrigger>
                  <TabsTrigger value="raw" className="data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-zinc-500 data-[state=active]:text-zinc-300 text-zinc-400 rounded-none h-11">
                ...
                <div className="flex-1 overflow-hidden">
                  ...
                  <TabsContent value="metadata" className="h-full m-0 p-0">
                     <ScrollArea className="h-full p-6">
                        <SessionMetadataView events={selectedSession.events} session={selectedSession} />
                     </ScrollArea>
                  </TabsContent>

                  <TabsContent value="findings" className="h-full m-0 p-0">
                    <ScrollArea className="h-full p-6">
                      {selectedSession.events.filter(e => e.findings && e.findings.length > 0).length === 0 ? (
                        ...
                        <div className="space-y-4">
                          {selectedSession.events.filter(e => e.findings && e.findings.length > 0).map((e, idx) => (
                            <div key={idx} className="space-y-3">
                              {e.findings!.map((finding, fIdx) => (
                                <div key={fIdx} className="bg-red-500/5 border border-red-500/20 rounded-lg p-4">
                                  <div className="flex justify-between items-start mb-2">
                                    <h4 className="text-red-400 font-bold flex items-center gap-2">
                                      <AlertTriangle className="w-4 h-4" />
                                      {finding.message}
                                    </h4>
                                    <Badge variant="outline" className="border-red-800 text-red-500 uppercase">
                                      {finding.severity}
                                    </Badge>
                                  </div>
                                  <p className="text-sm text-zinc-400 mb-3">Rule ID: <span className="font-mono text-zinc-300">{finding.ruleId}</span></p>
                                  {finding.snippet && (
...
function StatsCard({ title, value, subValue, icon, className }: StatsCardProps) {
  return (
...
function SessionMetadataView({ events, session }: { events: SessionEvent[], session: ReportSession }) {
   const metaEvent = events.find(e => e.type === 'session_meta');
   const turnContext = events.find(e => e.type === 'turn_context');
   const ghostSnapshot = events.find(e => e.type === 'response_item' && e.payload?.type === 'ghost_snapshot');
   ...
   return (
      <div className="space-y-8">
         ...
         <div className="space-y-3">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-zinc-950/50 p-3 rounded border border-zinc-800 text-sm space-y-2">
                   <div className="flex justify-between"><span className="text-zinc-500">Model:</span> <span className="text-zinc-200 font-mono">{context?.model || 'N/A'}</span></div>
                   <div className="flex justify-between"><span className="text-zinc-500">Provider:</span> <span className="text-zinc-200">{meta?.model_provider || 'N/A'}</span></div>
                   <div className="flex justify-between"><span className="text-zinc-500">Source:</span> <span className="text-zinc-200">{meta?.source || 'N/A'}</span></div>
                   <div className="flex justify-between"><span className="text-zinc-500">CLI Ver:</span> <span className="text-zinc-200 font-mono">{meta?.cli_version || 'N/A'}</span></div>
                </div>
                <div className="bg-zinc-950/50 p-3 rounded border border-zinc-800 text-sm space-y-2">
                   <div className="flex justify-between"><span className="text-zinc-500">Sandbox:</span> <span className="text-zinc-200">{context?.sandbox_policy?.type || 'N/A'}</span></div>
                   <div className="flex justify-between"><span className="text-zinc-500">Effort:</span> <span className="text-zinc-200">{context?.effort || 'N/A'}</span></div>
                   <div className="flex justify-between"><span className="text-zinc-500">Approval:</span> <span className="text-zinc-200">{context?.approval_policy || 'N/A'}</span></div>
                </div>
             </div>
         </div>
         ...
         <div className="space-y-3">
             <h3 className="text-sm font-medium text-zinc-200 flex items-center gap-2">
               <FolderKanban className="w-4 h-4 text-orange-400" /> Version Control (Ghost Commit)
             </h3>
             {git ? (
               <div className="bg-zinc-950/50 p-4 rounded border border-zinc-800 text-sm space-y-4">
                  <div className="grid grid-cols-1 gap-2">
                     <div className="flex items-center gap-2">
                        ...
                        <code className="text-orange-300 bg-orange-950/30 px-2 py-0.5 rounded border border-orange-900/50">{git.id}</code>
                     </div>
                     <div className="flex items-center gap-2">
                  ...
                  {git.preexisting_untracked_files && (
                     <div className="space-y-1">
                        <span className="text-zinc-500 text-xs uppercase tracking-wider">Untracked Files</span>
                        <div className="flex flex-wrap gap-1">
                           {git.preexisting_untracked_files.map((file: string, i: number) => (
                              <Badge key={i} variant="outline" className="border-zinc-800 text-zinc-400 font-mono text-xs">
                                 {file}
                              </Badge>
                           ))}
                        </div>
                ...
                <div className="text-zinc-500 italic text-sm">No version control snapshot available.</div>
             )}
         </div>
             ...
             {ratelimits ? (
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-zinc-950/50 p-3 rounded border border-zinc-800 text-sm">
                     ...
                     <div className="text-2xl font-bold text-white">{ratelimits.primary?.used_percent}%</div>
                     <div className="text-xs text-zinc-600">Resets: {new Date(ratelimits.primary?.resets_at * 1000).toLocaleTimeString()}</div>
                  </div>
                  <div className="bg-zinc-950/50 p-3 rounded border border-zinc-800 text-sm">
                     <div className="text-zinc-500 mb-1">Credits Balance</div>
                     <div className="text-2xl font-bold text-white">{ratelimits.credits?.balance}</div>
                     <div className="text-xs text-zinc-600">Has Credits: {ratelimits.credits?.has_credits ? 'Yes' : 'No'}</div>
                  </div>
               </div>
             ) : (
                <div className="text-zinc-500 italic text-sm">No usage data available.</div>
             )}
         </div>

         <div className="space-y-1 pt-4 border-t border-zinc-800">
            <h3 className="text-xs font-medium text-zinc-500 uppercase">Working Directory</h3>
            <code className="block bg-black/40 p-2 rounded text-xs text-zinc-400 font-mono break-all border border-zinc-800/50">
               ...
               {session.cwd || JSON.stringify((session.events.find(e => e.payload?.cwd) as any)?.payload?.cwd)}
            </code>
         </div>
      </div>
   );
  ...
  return (
    <>
      {sortedEvents.map((event, index) => {
        ...
        // 1. User Message
        if (type === 'user_message' || (event.type === 'response_item' && event.payload?.role === 'user')) {
          const content = event.payload.message || event.payload.content?.[0]?.text;
          ...
          return (
            <div key={index} className="flex justify-end pl-12">
              <div className="bg-blue-600/10 border border-blue-600/20 text-blue-100 rounded-lg rounded-tr-none p-4 max-w-full">
                <div className="text-xs text-blue-400/50 mb-1 font-mono uppercase">User Input</div>
        ...
        if (type === 'agent_reasoning' || (event.type === 'response_item' && event.payload?.type === 'reasoning')) {
            ...
            <div key={index} className="flex gap-3 pr-12">
               ...
               <div className="py-2">
                 <div className="text-zinc-400 text-sm italic border-l-2 border-purple-500/30 pl-4 py-1">
                    <span className="text-purple-400 text-xs font-bold uppercase not-italic block mb-1">Reasoning</span>
                    {text}
                 </div>
               </div>
            </div>
        ...
        if (type === 'agent_message' || (event.type === 'response_item' && event.payload?.role === 'assistant')) {
          const content = event.payload.message || event.payload.content?.[0]?.text;
          if (!content) return null;
        ...
        if (type === 'token_count') {
          ...
          return (
            <div key={index} className="flex justify-center my-4">
              <Badge variant="outline" className="text-xs font-mono text-zinc-600 border-zinc-800 bg-zinc-950/50">
                Tokens: {info.total_tokens} (In: {info.input_tokens} / Out: {info.output_tokens})
              </Badge>
            </div>
          );
        }
        ...
        if (event.findings && event.findings.length > 0) {
           return (
             <div key={index} className="my-2 border border-red-500/40 bg-red-500/5 rounded p-3 mx-8">
