...
const getFileIcon = (name: string, type: 'file' | 'folder') => {
  if (type === 'folder') return <Folder className="h-5 w-5 text-amber-500 fill-amber-500/20" />;
  if (name === 'report.json') return <ShieldAlert className="h-5 w-5 text-red-500" />;
  ...
  switch (ext) {
    ...
    case 'ts':
    case 'tsx':
    case 'jsx': return <FileCode className="h-5 w-5 text-blue-400" />;
    case 'json': return <Package className="h-5 w-5 text-yellow-400" />;
    case 'png':
    case 'jpg':
    case 'svg': return <ImageIcon className="h-5 w-5 text-purple-400" />;
    case 'md': return <FileText className="h-5 w-5 text-emerald-400" />;
...
const MarkdownComponents = {
  ...
  h1: (props: React.ComponentPropsWithoutRef<'h1'>) => (
    <h1 className="text-3xl font-bold text-zinc-100 mb-6 pb-4 border-b border-zinc-800 tracking-tight" {...props} />
  ),
  h2: (props: React.ComponentPropsWithoutRef<'h2'>) => (
    <h2 className="text-2xl font-semibold text-zinc-100 mb-4 mt-8 flex items-center gap-2" {...props} />
  ),
  ...
  ul: (props: React.ComponentPropsWithoutRef<'ul'>) => (
    <ul className="list-disc list-outside ml-5 text-zinc-400 mb-4 space-y-2 marker:text-emerald-500" {...props} />
  ),
  ol: (props: React.ComponentPropsWithoutRef<'ol'>) => (
    <ol className="list-decimal list-outside ml-5 text-zinc-400 mb-4 space-y-2 marker:text-zinc-500" {...props} />
  ),
  ...
  a: (props: React.ComponentPropsWithoutRef<'a'>) => (
    <a className="text-emerald-400 hover:text-emerald-300 hover:underline underline-offset-4 inline-flex items-center gap-0.5 transition-colors" target="_blank" rel="noopener noreferrer" {...props}>
      {props.children}
  ...
  // Style tekstu
  strong: (props: React.ComponentPropsWithoutRef<'strong'>) => <strong className="font-semibold text-zinc-200" {...props} />,
  em: (props: React.ComponentPropsWithoutRef<'em'>) => <em className="text-zinc-300 not-italic" {...props} />, // lekki wyróżnik zamiast standardowej kursywy
  ...
  code: ({
    ...
    children,
    ...props
  }: React.ComponentPropsWithoutRef<'code'> & { node?: unknown; inline?: boolean }) => {
    // Silencing unused variables
    void node;
    void className;
    if (inline) {
      return (
        <code className="bg-zinc-800/80 text-zinc-300 px-1.5 py-0.5 rounded text-[13px] font-mono border border-zinc-700/50" {...props}>
          {children}
        </code>
      );
    }
    return (
      <div className="my-6 rounded-lg overflow-hidden border border-zinc-800 bg-[#0e0e10] shadow-md">
        ...
        <div className="p-4 overflow-x-auto">
          <code className="text-[13px] font-mono text-zinc-300 block leading-6" {...props}>
            {children}
          </code>
        </div>
      </div>
  ...
  blockquote: (props: React.ComponentPropsWithoutRef<'blockquote'>) => (
    <blockquote className="border-l-4 border-emerald-500/30 pl-4 py-1 my-6 bg-emerald-500/5 rounded-r italic text-zinc-400" {...props} />
  ),
  
  // Obrazy
  img: (props: React.ComponentPropsWithoutRef<'img'>) => (
    // eslint-disable-next-line @next/next/no-img-element
    <img className="rounded-lg border border-zinc-800 my-6 max-w-full h-auto shadow-sm" {...props} alt={props.alt || 'Markdown image'} />
  ),
  ...
  th: (props: React.ComponentPropsWithoutRef<'th'>) => (
    <th className="bg-zinc-900 border-b border-zinc-800 p-3 text-zinc-400 font-medium uppercase tracking-wider text-xs" {...props} />
  ),
  td: (props: React.ComponentPropsWithoutRef<'td'>) => (
    <td className="border-b border-zinc-800/50 p-3 text-zinc-300 last:border-0" {...props} />
  ),
  tr: (props: React.ComponentPropsWithoutRef<'tr'>) => (
    <tr className="hover:bg-zinc-900/30 transition-colors even:bg-zinc-900/10" {...props} />
  ),
  
  // Linia pozioma
  hr: (props: React.ComponentPropsWithoutRef<'hr'>) => <hr className="my-8 border-zinc-800" {...props} />,
};
...
export function FileBrowser({ workspaceId, userId }: FileBrowserProps) {
  ...
  const fetchFiles = useCallback(async (path: string) => {
    ...
    setError(null);
    try {
      const result = await getUserFiles({ workspaceId, userId, prefix: path });
            ...
            <div className="flex items-center gap-1 overflow-x-auto scrollbar-hide px-2 font-mono text-sm">
               <button
                  ...
                  className={cn(
                     ...
                     !currentPath ? "text-white font-medium bg-zinc-800/50" : "text-zinc-500"
                  )}
               >
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 overflow-hidden shadow-sm min-h-[400px] flex flex-col">
        {loading ? (
           ...
           <div className="flex-1 flex items-center justify-center flex-col gap-4 text-red-400">
              ...
              <Button variant="outline" size="sm" onClick={() => fetchFiles(currentPath)} className="border-red-900/50 hover:bg-red-950">
                 Try Again
              </Button>
           </div>
        ) : (
           <Table>
              <TableHeader className="bg-zinc-950/50 border-b border-zinc-800">
                 <TableRow className="border-zinc-800 hover:bg-transparent">
                    ...
                    <TableHead className="text-zinc-400 text-xs uppercase tracking-wider font-medium">Name</TableHead>
                    <TableHead className="text-zinc-400 text-xs uppercase tracking-wider font-medium w-[120px]">Size</TableHead>
                    <TableHead className="text-zinc-400 text-xs uppercase tracking-wider font-medium text-right pr-4 w-[150px]">Last Modified</TableHead>
                 </TableRow>
              </TableHeader>
              <TableBody>
                    ...
                    filteredItems.map((item) => (
                       ...
                       >
                          ...
                          <TableCell className="py-3 text-zinc-500 text-sm font-mono">
                             {item.type === 'folder' ? '-' : item.size ? (item.size / 1024).toFixed(1) + ' KB' : '0 KB'}
                          </TableCell>
                          <TableCell className="py-3 text-zinc-500 text-sm font-mono text-right pr-4">
                             {item.lastModified ? formatDistanceToNow(new Date(item.lastModified), { addSuffix: true }) : '-'}
                          </TableCell>
                       </TableRow>
                    ))
                 )}
           ...
           <DialogHeader className="px-6 py-4 border-b border-zinc-800/80 bg-zinc-900/50 backdrop-blur-md flex flex-row items-center justify-between space-y-0 z-10">
             <div className="flex items-center gap-4">
               ...
               <div className="flex flex-col gap-0.5">
                 <DialogTitle className="text-zinc-100 font-semibold tracking-tight">{viewingFile?.name}</DialogTitle>
                 <div className="flex items-center gap-2 text-xs text-zinc-500 font-mono">
                    <span>{viewingFile?.size ? (viewingFile.size / 1024).toFixed(2) + ' KB' : '0 KB'}</span>
           ...
           <div className="flex-1 overflow-hidden relative bg-[#09090b]">
              ...
              {loadingContent ? (
                 <div className="absolute inset-0 flex items-center justify-center flex-col gap-4 text-zinc-500">
                    <Loader2 className="h-8 w-8 animate-spin text-emerald-500" />
                    <p className="text-sm font-medium animate-pulse">Rendering markdown...</p>
                 </div>
              ) : (
